program test_riscv_source
    ...
    type(riscv_opcode_record_t) :: records(3)
    ...
    call assert_equal_int(count, 3_int32, 'bounded importer changed record count')
    ...
    call assert_equal(trim(records(3)%mnemonic), 'addi', 'addi was not normalized')
    ...
    call assert_equal64(records(3)%match, int(z'00000013', int64), 'addi match changed')
    call assert_equal64(records(1)%mask, int(z'FE00707F', int64), 'add mask changed')
    call assert_equal64(records(3)%mask, int(z'0000707F', int64), 'addi mask changed')
