program test_riscv_fixture
    ...
    use fortback_riscv_fixture, only: riscv_add, riscv_addi, riscv_decode_integer, &
    ...
    target = make_target_ir('riscv64', 64_int32, .true., &
        make_source_ref('riscv-opcodes', 'rv64i', 'fixture-sha256'))

    instruction = riscv_instruction_t(riscv_add, 10_int32, 10_int32, 11_int32, 0_int32)
    call riscv_encode_integer(target, instruction, word, status)
    ...
    call riscv_decode_integer(target, int(z'00001013', int64), decoded, status)
    call assert_equal_integer(status, riscv_unsupported, 'unsupported instruction accepted')
    call riscv_decode_integer(target, int(z'100000000', int64), decoded, status)
    ...
    call riscv_encode_integer(bad_target, instruction, word, status)
...
contains
    ...
    subroutine check_decode(target, word, expected, message)
        ...
        call riscv_decode_integer(target, word, decoded, status)
