program test_riscv_fixture
    ...
    use fortback_riscv_fixture, only: riscv_add, riscv_addi, riscv_and, &
        ...
        riscv_slti, riscv_sltiu, riscv_unsupported, &
    ...
    type(riscv_opcode_record_t) :: records(16)
    ...
    character(len=*), parameter :: source_text = &
        ...
        'sltiu rd rs1 imm12 14..12=3 6..2=0x04 1..0=3' // new_line('a') // &
    ...
    call assert_equal_integer(count, 16_int32, 'source record count changed')
    call assert_equal(trim(records(15)%source%artifact), trim(target%source%artifact), &
        'sltiu source artifact provenance was lost')
    call assert_equal(trim(records(15)%source%object), trim(target%source%object), &
        'sltiu source object provenance was lost')
    call assert_equal(trim(records(15)%source%source_hash), trim(target%source%source_hash), &
        'sltiu source hash provenance was lost')
    call assert_equal(trim(records(15)%source%origin), trim(target%source%origin), &
        'sltiu source origin provenance was lost')
    call assert_equal(trim(records(16)%source%artifact), trim(target%source%artifact), &
        'xori source artifact provenance was lost')
    call assert_equal(trim(records(16)%source%object), trim(target%source%object), &
        'xori source object provenance was lost')
    call assert_equal(trim(records(16)%source%source_hash), trim(target%source%source_hash), &
        'xori source hash provenance was lost')
    call assert_equal(trim(records(16)%source%origin), trim(target%source%origin), &
    ...
    instruction = riscv_instruction_t(riscv_sltiu, 10_int32, 10_int32, 0_int32, -1_int32)
    call riscv_encode_integer(target, instruction, word, status, records)
    call assert_equal_integer(status, riscv_ok, 'sltiu was rejected')
    call assert_equal_integer64(word, int(z'FFF53513', int64), 'sltiu encoding changed')
    call check_decode(target, word, instruction, 'sltiu decode')

    instruction = riscv_instruction_t(riscv_xori, 10_int32, 10_int32, 0_int32, -1_int32)
    call riscv_encode_integer(target, instruction, word, status, records)
    call assert_equal_integer(status, riscv_ok, 'xori was rejected')
    call assert_equal_integer64(word, int(z'FFF54513', int64), 'xori encoding changed')
    call check_decode(target, word, instruction, 'xori decode')
    ...
    instruction = riscv_instruction_t(riscv_xori, 1_int32, 1_int32, 0_int32, -2049_int32)
    call riscv_encode_integer(target, instruction, word, status, records)
    call assert_equal_integer(status, riscv_invalid_operand, 'wide xori immediate accepted')
    ...
    call riscv_decode_integer(target, int(z'FFF55513', int64), decoded, status, records)
    call assert_equal_integer(status, riscv_unsupported, 'unsupported I-format encoding accepted')
    ...
    call riscv_encode_integer(bad_target, riscv_instruction_t(riscv_sltiu, 1_int32, 2_int32, &
        0_int32, 2047_int32), word, status, records)
    call assert_equal_integer(status, riscv_invalid_target, 'wrong target accepted SLTIU')
    call riscv_decode_integer(bad_target, int(z'7FF13093', int64), decoded, status, records)
    call assert_equal_integer(status, riscv_invalid_target, 'wrong target decoded SLTIU')
    call riscv_encode_integer(bad_target, riscv_instruction_t(riscv_xori, 1_int32, 2_int32, &
        0_int32, 2047_int32), word, status, records)
    call assert_equal_integer(status, riscv_invalid_target, 'wrong target accepted XORI')
    call riscv_decode_integer(bad_target, int(z'7FF14093', int64), decoded, status, records)
    call assert_equal_integer(status, riscv_invalid_target, 'wrong target decoded XORI')
