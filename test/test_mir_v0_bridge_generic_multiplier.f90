program test_mir_v0_bridge_generic_multiplier
    ...
    input = initialized_mul_input(42, 0)
    call assert_rejected(input, 'multiplier literal below the accepted bound was accepted')
    input = initialized_mul_input(42, 11)
...
contains
    ...
    subroutine assert_qemu(left, multiplier, expected)
        integer, intent(in) :: left, multiplier
        character(len=*), intent(in) :: expected
        character(len=16) :: bytes
        integer :: byte_index

        input = initialized_mul_input(left, multiplier)
        call compile_mir_v0_riscv_linux(input, artifact, status, diagnostic)
        call assert_equal(status, mir_v0_bridge_ok, &
            'accepted initialized multiplier was rejected')
        call assert_true(len_trim(diagnostic) == 0, &
            'accepted initialized multiplier returned a diagnostic')
        call assert_true(riscv_linux_artifact_provenance_valid(artifact), &
            'initialized multiplier artifact provenance is invalid')
        call write_mir_v0_riscv_linux(input, elf_path, status, diagnostic)
        call assert_equal(status, mir_v0_bridge_ok, &
            'initialized multiplier ELF write failed')
        call execute_command_line('chmod 755 -- '//elf_path, wait=.true., &
            exitstat=exit_status, cmdstat=command_status)
        call assert_equal(command_status, 0, 'initialized multiplier chmod failed')
        call assert_equal(exit_status, 0, 'initialized multiplier chmod returned failure')
        call execute_command_line('qemu-riscv64 '//elf_path//' > '//output_path, wait=.true., &
            exitstat=exit_status, cmdstat=command_status)
        call assert_equal(command_status, 0, 'initialized multiplier QEMU command failed')
        call assert_equal(exit_status, 0, 'initialized multiplier QEMU returned failure')
        open (newunit=unit, file=output_path, access='stream', form='unformatted', &
            status='old', action='read', iostat=io_status)
        call assert_equal(io_status, 0, 'initialized multiplier output was not written')
        read (unit, iostat=io_status) bytes(1:len(expected))
        call assert_equal(io_status, 0, 'initialized multiplier output read failed')
        do byte_index = 1, len(expected)
            call assert_byte(bytes(byte_index:byte_index), expected(byte_index:byte_index), &
                'initialized multiplier output changed')
        end do
        read (unit, iostat=io_status) bytes(1:1)
        call assert_true(io_status /= 0, 'initialized multiplier wrote extra output')
        close (unit, status='delete', iostat=io_status)
        call assert_equal(io_status, 0, 'initialized multiplier output cleanup failed')
    ...
    subroutine assert_rejected(value, message)
        ...
        call assert_true(len_trim(diagnostic) > 0, &
            'rejected initialized multiplier returned no diagnostic')
    ...
    function initialized_mul_input(left, multiplier) result(value)
        integer, intent(in) :: left, multiplier
    ...
    end function initialized_mul_input
...
end program test_mir_v0_bridge_generic_multiplier
