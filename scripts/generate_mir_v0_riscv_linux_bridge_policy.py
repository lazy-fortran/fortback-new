    ...
    for line_number, line in enumerate(INPUT.read_text().splitlines(), 1):
        ...
        if (fields[0] == "storage-policy" and len(fields) == 13 and fields[1] == "key" and
            ...
            if fields[6] == "stored-variable-print":
                counts = range(count, 208, 2) if count == 49 else (count,)
...
def render(policy):
    ...
    lines += [
        ...
        "            if (index <= 1_int32) then",
        "                operation = 'addi'",
        "            else if (index <= 11_int32) then",
        "                if (mod(index / 2_int32, 2_int32) == 1_int32) then",
        "                    operation = 'sb'",
        "                else",
        "                    operation = 'addi'",
        "                end if",
        "            else if (mod(index, 2_int32) == 0_int32) then",
        "                operation = 'addi'",
        "            else",
        "                operation = 'sb'",
        "            end if",
