...
def read_policy():
    policy = {"instruction-count": None, "instructions": [], "source-rules": []}
    result = {}
    for line_number, line in enumerate(INPUT.read_text().splitlines(), 1):
        ...
        if fields[0] == "instruction-count" and len(fields) == 3 and fields[1] == "value":
            ...
            if fields[1] in result:
                raise SystemExit(f"{INPUT}:{line_number}: duplicate result field")
            result[fields[1]] = fields[2]
        elif fields[0] == "source-rule" and len(fields) == 4 and fields[1] == "function":
            policy["source-rules"].append((fields[2], fields[3]))
        else:
    ...
    if set(result) != {"id", "kind", "type"}:
        raise SystemExit(f"{INPUT}: result fields must be id, kind, and type")
...
def render(policy, result):
    lines = [
        ...
        f"    integer(int32), parameter, public :: mir_v0_bridge_policy_result_id = {result['id']}_int32",
        "    integer(int32), parameter, public :: mir_v0_bridge_policy_result_kind = &",
        "        mir_v0_value_kind_integer",
        f"    character(len=*), parameter, public :: mir_v0_bridge_policy_result_type = '{result['type']}'",
        "",
    ...
    for function_name, _ in policy["source-rules"]:
    ...
    for function_name, source_rule in policy["source-rules"]:
        lines += [f"        case ('{function_name}')", f"            if (trim(source_rule) /= '{source_rule}') return"]
...
def main():
    ...
    generated = render(policy, result)
