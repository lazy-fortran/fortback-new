...
contains
    subroutine compile_mir_v0_riscv_linux(input, artifact, status, diagnostic)
        ...
        if (print_variable_three_item_route) then
            ...
            call encode_operation(target, records, 'sb', [5_int64, 2_int64, 5_int64], words(22), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [10_int64, 0_int64, 1_int64], words(23), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [11_int64, 2_int64, 0_int64], words(24), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [12_int64, 0_int64, 6_int64], words(25), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [17_int64, 0_int64, 64_int64], words(26), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, mir_v0_riscv_linux_ecall_operation, &
                mir_v0_riscv_linux_ecall_operands, words(27), status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [10_int64, 0_int64, 0_int64], words(28), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, 'addi', [17_int64, 0_int64, 93_int64], words(29), &
                status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            call encode_operation(target, records, mir_v0_riscv_linux_ecall_operation, &
                mir_v0_riscv_linux_ecall_operands, words(30), status, diagnostic)
            if (status /= mir_v0_bridge_ok) return
            emitted_count = 30_int32
