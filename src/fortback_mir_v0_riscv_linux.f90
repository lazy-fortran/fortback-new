module fortback_mir_v0_riscv_linux
    ...
    integer, parameter :: instruction_capacity = 128
...
contains
    subroutine compile_mir_v0_riscv_linux(input, artifact, status, diagnostic)
        ...
        logical :: print_variable_seven_to_sixty_item_route
        ...
        print_variable_seven_to_sixty_item_route = &
            is_print_variable_seven_to_sixty_item_candidate(mir)
        ...
        if (print_variable_seven_to_sixty_item_route) then
            call encode_print_variable_seven_to_sixty(target, records, mir, words, emitted_count, &
                status, diagnostic)
            ...
            if (mir%instructions(5)%opcode == mir_v0_opcode_pow) then
                call encode_operation(target, records, 'mul', [11_int64, 10_int64, 10_int64], words(6), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'mul', [10_int64, 11_int64, 10_int64], words(7), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, trim(mir_v0_bridge_policy_store_operation), &
                    [10_int64, 2_int64, int(mir_v0_bridge_policy_storage_offset, int64)], words(8), &
                ...
                call encode_operation(target, records, 'sb', [5_int64, 2_int64, 0_int64], words(11), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'addi', [5_int64, 0_int64, 10_int64], words(12), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'sb', [5_int64, 2_int64, 1_int64], words(13), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'addi', [10_int64, 0_int64, 1_int64], words(14), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'addi', [11_int64, 2_int64, 0_int64], words(15), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'addi', [12_int64, 0_int64, 2_int64], words(16), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, 'addi', [17_int64, 0_int64, 64_int64], words(17), &
                    status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, mir_v0_riscv_linux_ecall_operation, &
                    mir_v0_riscv_linux_ecall_operands, words(18), status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, trim(mir_v0_bridge_policy_exit_status_operation()), &
                    [10_int64, 0_int64, 0_int64], words(19), status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, trim(mir_v0_bridge_policy_exit_status_operation()), &
                    [17_int64, 0_int64, 93_int64], words(20), status, diagnostic)
                if (status /= mir_v0_bridge_ok) return
                call encode_operation(target, records, mir_v0_riscv_linux_ecall_operation, &
    ...
    subroutine encode_print_variable_seven_to_sixty(target, records, mir, words, emitted_count, &
            status, diagnostic)
    ...
    end subroutine encode_print_variable_seven_to_sixty
    ...
    logical function validate_scope(mir, status, diagnostic) result(ok)
        ...
        if (print_variable_seven_to_sixty_item_route) then
            if (.not. valid_print_variable_seven_to_sixty_item(mir)) then
                call set_diagnostic(diagnostic, &
    ...
    logical function is_print_variable_seven_to_sixty_item_candidate(mir) result(candidate)
        type(parsed_mir_t), intent(in) :: mir
        integer :: index, item_count

        candidate = .false.
        if (trim(mir%name) /= 'main') return
        if (mir%instruction_count < 21_int32 .or. mir%instruction_count > 127_int32) return
        ...
        candidate = .true.
    end function is_print_variable_seven_to_sixty_item_candidate
    ...
    logical function valid_print_variable_seven_to_sixty_item(mir) result(valid)
        type(parsed_mir_t), intent(in) :: mir
        integer :: index, item_count
        ...
        valid = .true.
    end function valid_print_variable_seven_to_sixty_item
